...
import { X, Calendar, FileText, MessageSquare, Info, User } from "lucide-react";
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      ...
      >
        ...
        <div className="p-8 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-1">
              ...
              <div className="h-11 flex items-center px-4 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-700 capitalize">
                {leave.type}
              </div>
            </div>
            ...
            <div className="space-y-1">
              ...
              <div className="h-11 flex items-center px-4 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-700 tabular-nums">
                {leave.fromDate.slice(0, 10)} <span className="mx-2 text-slate-300">→</span> {leave.toDate.slice(0, 10)}
              </div>
            </div>
          </div>
          ...
          {/* ADMIN COMMENT */}
          {leave.adminComment && (
            <div className={`${current.bg} ${current.border} p-4 rounded-xl border`}>
        ...
        <div className="px-8 py-5 bg-slate-50 border-t border-slate-100 flex justify-end">
          <button
            onClick={onClose}
            className="px-6 h-10 bg-white border border-slate-200 text-slate-600 text-sm font-bold rounded-lg hover:bg-slate-100 transition-colors shadow-sm"
          >
            Dismiss
          </button>
        </div>
